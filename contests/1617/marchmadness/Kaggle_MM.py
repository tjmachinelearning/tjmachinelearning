...
def create_model(num_teams):
	model = Sequential() #standard model
	model.add(Embedding(input_dim=num_teams, output_dim=50, input_length=2, init='uniform', trainable=True))
	model.add(Flatten()) #turns it all into 1D / linear
	model.add(Dense(100))
